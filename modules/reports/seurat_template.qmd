...
if (file.exists(comparison_metrics_path)) {
  ...
  cat("This section compares droplet calling between Cell Ranger and CellBender.\\n\\n")
  
  # Display metrics table
  knitr::kable(
    comparison_metrics,
    caption = "Droplet Calling Statistics",
    format = "html"
  )
  ...
  cat("\\n**Key Findings:**\\n\\n")
  cat("- Total raw droplets sequenced:", format(raw_droplets, big.mark = ","), "\\n")
  cat("- Cell Ranger called:", format(cr_cells, big.mark = ","), "cells\\n")
  
  if (!is.na(cb_cells)) {
    cat("- CellBender called:", format(cb_cells, big.mark = ","), "cells\\n")
    diff <- cb_cells - cr_cells
    diff_pct <- round(100 * diff / cr_cells, 2)
    if (diff > 0) {
      cat("- CellBender called", format(abs(diff), big.mark = ","), "**more** cells than Cell Ranger (", diff_pct, "% increase)\\n")
    } else if (diff < 0) {
      cat("- CellBender called", format(abs(diff), big.mark = ","), "**fewer** cells than Cell Ranger (", abs(diff_pct), "% decrease)\\n")
    } else {
      cat("- CellBender and Cell Ranger called the same number of cells\\n")
    }
  }
...
if (file.exists(comparison_plot_path)) {
  ...
  cat("The knee-plot shows the distribution of UMI counts across all droplets, colored by which droplets were called as cells by Cell Ranger (blue) vs CellBender (purple).\\n\\n")
