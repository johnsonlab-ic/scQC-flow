...
```{r}
#| label: cellbender-status
#| results: 'asis'
#| echo: false

# Load CellBender comparison metrics
comparison_metrics_path <- "COMPARISON_METRICS_PLACEHOLDER"
comparison_plot_path <- "COMPARISON_PLOT_PLACEHOLDER"

# Check if files exist
if (file.exists(comparison_metrics_path)) {
  comparison_metrics <- read.csv(comparison_metrics_path)
  
  # Display key findings and difference flag first
  raw_droplets <- comparison_metrics$count[comparison_metrics$metric == "Total Raw Droplets"]
  cr_cells <- comparison_metrics$count[comparison_metrics$metric == "CellRanger Called Cells"]
  cb_cells <- comparison_metrics$count[comparison_metrics$metric == "CellBender Called Cells"]
  
  # Calculate difference percentage for status indicator
  if (!is.na(cb_cells) && cr_cells > 0) {
    diff <- cb_cells - cr_cells
    diff_pct <- abs(round(100 * diff / cr_cells, 2))
    
    # Determine status text (no HTML to avoid rendering issues)
    if (diff_pct <= 5) {
      status_emoji <- "✅"
      status_label <- "OK"
      message <- "CellBender and Cell Ranger results are consistent (< 5% difference)"
    } else if (diff_pct <= 20) {
      status_emoji <- "⚠️"
      status_label <- "Warning"
      message <- paste0("Moderate difference detected (", diff_pct, "% difference)")
    } else {
      status_emoji <- "🔴"
      status_label <- "Attention"
      message <- paste0("Large difference detected (", diff_pct, "% difference) - Review recommended")
    }
    
    cat(paste0("**CellBender vs Cell Ranger:** ", status_emoji, " ", status_label, " — ", message, "\n\n"))
  }
}
```

## Droplet Calling Comparison

This section compares droplet calling between Cell Ranger and CellBender.

```{r}
#| label: cellbender-comparison-table
#| echo: false
...
if (file.exists(comparison_metrics_path)) {
  ...
  if (!is.na(cb_cells)) {
    diff <- cb_cells - cr_cells
    diff_pct <- round(100 * diff / cr_cells, 2)
    
    comparison_summary <- data.frame(
      Method = c("Cell Ranger", "CellBender", "Difference"),
      `Cells Called` = c(
        format(cr_cells, big.mark = ","),
        format(cb_cells, big.mark = ","),
        format(diff, big.mark = ",")
      ),
      `% of Cell Ranger` = c(
        "100%",
        sprintf("%.2f%%", 100 * cb_cells / cr_cells),
        sprintf("%+.2f%%", diff_pct)
      ),
      `% of Total Droplets` = c(
        sprintf("%.2f%%", 100 * cr_cells / raw_droplets),
        sprintf("%.2f%%", 100 * cb_cells / raw_droplets),
        ""
      ),
      check.names = FALSE
    )
    ...
    comparison_summary <- data.frame(
      Method = c("Cell Ranger"),
      `Cells Called` = c(format(cr_cells, big.mark = ",")),
      `% of Total Droplets` = c(sprintf("%.2f%%", 100 * cr_cells / raw_droplets)),
      check.names = FALSE
    )
  }
  ...
  knitr::kable(
    comparison_summary,
    caption = paste0("Droplet Calling Summary (Total Droplets: ", format(raw_droplets, big.mark = ","), ")"),
    format = "html",
    align = c("l", "r", "r", "r")
  )
} else {
  cat("\n*CellBender comparison metrics not available.*\n\n")
}
```

## Knee-Plot Comparison

The knee-plots show the distribution of UMI counts across all droplets. Each plot highlights the droplets called as cells by Cell Ranger (left, blue) and CellBender (right, purple), with background droplets shown in gray.

```{r}
#| label: cellbender-kneeplot
#| fig-width: 18
#| fig-height: 10
#| echo: false

# Display knee-plot if available
if (file.exists(comparison_plot_path)) {
  knitr::include_graphics(comparison_plot_path)
} else {
  cat("\n*Knee-plot not available.*\n\n")
}
```
