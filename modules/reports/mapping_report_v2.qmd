---
title: "Read mapping diagnostics (DropletUtils 1.26)"
format:
  html:
    toc: true
    toc-depth: 3
...
```{r load_data}
knee_fs <- list.files(".", pattern = "^knee_plot_data_v2_.+\\.csv$",
                      full.names = TRUE, recursive = FALSE)

if (length(knee_fs) == 0) stop("No knee_plot_data_v2_*.csv files found")

sample_var <- "sample_id"

ids    <- sub("^knee_plot_data_v2_(.+)\\.csv$", "\\1", basename(knee_fs))
...
knitr::kable(called_cells, digits = 2,
  caption = "DropletUtils 1.26 — per-sample droplet and called-cell summary")
```

## Barcode rank plots {.tabset}

### By slope ratio {.tabset}
...
### By expected/total ratio {.tabset}
